...
const baseUrl = 'https://pcdownload-huoshan.aki-game.com/launcher/game/G152/2.2.0/ifrkqjQOAZQkvNXJcAgTKlbRegvLoMZH/zip/';
...
function log(index, total, msg) {
  const prefix = index !== null ? `[${String(index + 1).padStart(3, '0')}/${total}]` : '';
    ...
    https.get(url, res => {
      ...
      res.on('data', chunk => {
        ...
        if (nowTime - lastLogged >= 1000) {
          const speed = formatSpeed((downloaded - lastDownloaded) / ((nowTime - lastLogged) / 1000));
          const totalMB = (expectedSize / 1048576).toFixed(2);
          const downloadedMB = (downloaded / 1048576).toFixed(2);
          log(index, total, `下载 ${name} 进度: ${downloadedMB} MB / ${totalMB} MB (${speed})`);
          lastLogged = nowTime;
  ...
  return new Promise((resolve, reject) => {
    ...
    stream.on('data', chunk => {
      ...
      if (fileSize > 1048576) {
        ...
        const nowTime = Date.now();
        if (nowTime - lastLogged >= 1000) {
          const speed = formatSpeed((processed - lastProcessed) / ((nowTime - lastLogged) / 1000));
  ...
  const checkAndDownload = async (retry = false) => {
    if (fs.existsSync(fullPath)) {
      ...
      if (stats.size === fileObj.size) {
        const md5 = await getFileMD5WithProgress(fullPath, index, total, name, stats.size);
        if (md5 === fileObj.md5) {
    ...
    try {
      await downloadFileWithProgress(url, fullPath, index, total, name, fileObj.size);
