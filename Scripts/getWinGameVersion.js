...
process.argv[2] = 'cn'
// 根据命令行参数选择目标链接
const targetUrl = process.argv[2] === 'cn' ? CN_API_URL : process.argv[2] === 'os' ? OS_API_URL : (() => { throw new Error('无效的命令行参数: ' + process.argv[2]); })();
const targetDir = process.argv[2] === 'cn' ? './Win/Game/CN/' : './Win/Game/OS/';
const latestVerPath = process.argv[2] === 'cn' ? './Scripts/data/latest_Win_Game_CN.json' : './Scripts/data/latest_Win_Game_OS.json';
...
async function getWinGameVersion() {
    try {
        ...
        console.log('本地最新 REL 版本:', localData.latestVersion, '预下载版本:', localData.preDownloadVersion)
        console.log('取到最新 REL 版本:', latestVersion, '预下载版本:', preDownloadVersion);

        // 构建文件名
        const fileName = `REL${latestVersion}` + (preDownloadVersion ? `+PRE${preDownloadVersion}` : '') + '.json';
        //console.log('文件名:', fileName);

        // 比较版本并更新本地数据
        if (localData.latestVersion !== latestVersion || localData.preDownloadVersion !== preDownloadVersion) {
