   ...
   */
    function getEncodedLinkMsgGroupText(server, linkType, updateType) {
        ...
        try {
            ...
            const jsonDataContent = fs.readFileSync(
                `./Win/Game/${server}/${getLatestJsonFileName(`./Win/Game/${server}/`)}`,
                'utf-8'
            )
        ...
        let linkData = updateType === 'rel' ? jsonData.data.game : jsonData.data.pre_download_game
        //console.log('linkData:', linkData)
        let fullLink = '本体: 分卷 '
        if(linkType === 'full') {
            
            for(let i = 0; i < linkData.latest.segments.length; i++) {
                fullLink += `[${i+1}](${escapeCharacters(linkData.latest.segments[i].path)})\\|`
            }
            // 去掉最后一个 |
            ...
            for(let i = 0; i < voiceData.length; i++) {
                if (voiceData[i].language === 'zh-cn') voiceLink += `[中文](${escapeCharacters(voiceData[i].path)})\\|`
                else if (voiceData[i].language === 'en-us') voiceLink += `[英文](${escapeCharacters(voiceData[i].path)})\\|`
                else if (voiceData[i].language === 'ja-jp') voiceLink += `[日文](${escapeCharacters(voiceData[i].path)})\\|`
                else if (voiceData[i].language === 'ko-kr') voiceLink += `[韩文](${escapeCharacters(voiceData[i].path)})\\|`
            }
            // 去掉最后一个 |
            ...
            for(let i = 0; i < linkData.diffs.length; i++) {
                diffLink += `${escapeCharacters(linkData.diffs[i].version)}\\-${escapeCharacters(linkData.latest.version)}: `
                ...
                for(let j = 0; j < voiceData.length; j++) {
                    if(voiceData[j].language === 'zh-cn') diffLink += `[中文](${escapeCharacters(voiceData[j].path)})\\|`
                    else if(voiceData[j].language === 'en-us') diffLink += `[英文](${escapeCharacters(voiceData[j].path)})\\|`
                    else if(voiceData[j].language === 'ja-jp') diffLink += `[日文](${escapeCharacters(voiceData[j].path)})\\|`
                    else if(voiceData[j].language === 'ko-kr') diffLink += `[韩文](${escapeCharacters(voiceData[j].path)})\\|`
                }
                // 去掉最后一个 |
    ...
    function getLatestJsonFileName(directoryPath) {
        ...
        const latestJsonFile = jsonFiles.reduce((latestFile, currentFile) => {
          ...
          return fs.statSync(currentFilePath).mtime > fs.statSync(latestFilePath).mtime ? currentFile : latestFile;
        });
